...
void CpuReducer::axpy(void* dst, const void* src, size_t len, DataType dtype,
                      float alpha) {
  switch (dtype) {
    case BYTEPS_FLOAT32:
      return cblas_saxpy(len / sizeof(float), alpha,
                         reinterpret_cast<const float*>(src), 1,
                         reinterpret_cast<float*>(dst), 1);
    case BYTEPS_FLOAT64:
      return cblas_daxpy(len / sizeof(double), alpha,
                         reinterpret_cast<const double*>(src), 1,
